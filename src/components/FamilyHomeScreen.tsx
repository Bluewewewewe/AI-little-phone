      ...
      <div className="flex-1 overflow-auto p-4 space-y-3">
        ...
        <div className="glass-card p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[11px] font-medium text-amber-900/50 uppercase tracking-widest">🏡 户型图</h3>
            <span className="text-[9px] text-amber-800/25">{isNight ? '🌙 深夜' : isEvening ? '🌆 傍晚' : '☀️ 白天'}</span>
          </div>
            ...
            {rooms.map(room => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] text-amber-800/25">{room.name}</span>
                </div>
        ...
        {showPeek && peekTarget && (
          <div className="glass-card p-4 border-pink-400/10 animate-fadeIn">
            ...
            <p className="text-sm text-amber-800/60 leading-relaxed">{peekDetail}</p>
            <button onClick={() => setShowPeek(false)} className="mt-2.5 text-[11px] text-amber-800/25 hover:text-amber-900/50 transition-colors">关闭</button>
          </div>
        )}
...
function ParentCard({ status, color, onPeek, onChat }: {
  ...
  return (
    <div className={`glass-card p-3 bg-gradient-to-br ${c.bg} ${c.accent} border text-center`}>
      ...
      <p className="text-[11px] text-amber-900/55 mt-1.5">{status.emoji} {status.activity}</p>
